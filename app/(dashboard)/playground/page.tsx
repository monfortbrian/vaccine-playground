...
export default function PlaygroundPage() {
  ...
  const handleRun = async () => {
    ...
    try {
      await supabase.auth.getSession(); // refresh session before run

      const body: Record<string, unknown> = {
        input_type: inputType,
        input_value: inputValue.trim(),
        run_safety: runSafety,
        run_coverage: runCoverage,
      };
      if (inputType === "pathogen") body.max_proteins = maxProteins;
      if (inputType === "sequence" && proteinName.trim()) {
        body.protein_name = proteinName.trim();
      }
  ...
  const handleStop = () => {
    ...
    // TODO: api.cancelRun(runId) when backend endpoint is added
  };

  // ── Status badge variant ───────────────────────────────────────────────────

  function statusVariant(s: string) {
    if (s === "running" || s === "pending") return "default" as const;
    ...
    <div className="space-y-6">
      ...
      <div className="grid gap-6 lg:grid-cols-[400px_1fr]">
        ...
        <div className="flex flex-col gap-4">
          <Card>
            <CardHeader className="pb-4">
              ...
              <CardDescription>
                Select input type and configure run parameters.
              </CardDescription>
            </CardHeader>
            <CardContent className="space-y-5">
              ...
              {inputType === "sequence" ? (
                <div className="space-y-4">
                  <div className="space-y-2">
                    ...
                    <Textarea
                      className="font-mono text-xs min-h-[120px] resize-none leading-relaxed"
              ...
              {/* Max proteins slider — pathogen only */}
              {inputType === "pathogen" && (
                <>
                  <Separator />
                    ...
                    <div className="flex items-center justify-between">
                      <div>
                        ...
                        <p className="text-[11px] text-muted-foreground mt-0.5">
                          Top surface-exposed candidates
                        </p>
                      </div>
                      <span className="rounded-md bg-primary text-primary-foreground px-2.5 py-0.5 font-mono text-xs font-semibold tabular-nums">
                        {maxProteins}
                      </span>
                    </div>
                    <input
                      ...
                      disabled={running}
                      className="w-full accent-primary h-1.5 rounded-full cursor-pointer disabled:opacity-40"
                    />
              ...
              <div className="space-y-4">
                <div className="flex items-center justify-between gap-4">
                  <div>
                    ...
                    <p className="text-[11px] text-muted-foreground mt-0.5">
                      Allergenicity &amp; toxicity — N6
                    </p>
                  </div>
                  <Switch
                    checked={runSafety}
                    onCheckedChange={setRunSafety}
                    disabled={running}
                  />
                </div>
                <div className="flex items-center justify-between gap-4">
                  <div>
                    ...
                    <p className="text-[11px] text-muted-foreground mt-0.5">
                      HLA coverage across 7 populations — N7
                    </p>
                  </div>
                  <Switch
                    checked={runCoverage}
                    onCheckedChange={setRunCoverage}
                    disabled={running}
                  />
                </div>
              </div>
        ...
        <div className="flex flex-col gap-4">
            ...
            <Card className="border-destructive/30 bg-destructive/5">
              <CardContent className="flex items-start gap-3 pt-5 pb-5">
                ...
                <div className="space-y-1">
                  ...
                  <code className="block rounded-md bg-muted px-3 py-2 text-xs font-mono mt-1">
          ...
          {/* Idle / empty state */}
          {!running && !ps && !error && (
            <div className="rounded-xl border border-dashed border-border flex flex-col items-center justify-center py-16 px-8 text-center gap-8">
              <div className="space-y-2">
                <h3 className="text-base font-semibold">Ready to analyze</h3>
                <p className="text-sm text-muted-foreground max-w-sm leading-relaxed">
                  The 8-node pipeline runs from proteome curation through
                  ...
                  <div>
                    ...
                    <CardDescription className="mt-1">
                      {ps.status === "paused"
                        ...
                        : "Running TOPE_DEEP N1 → N8."}
                    </CardDescription>
                  </div>
                  <div className="flex items-center gap-2 shrink-0">
                    <Badge variant={statusVariant(ps.status)} className="text-xs">
